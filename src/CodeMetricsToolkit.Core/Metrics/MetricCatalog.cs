...
public static class MetricCatalog
{
    ...
    [
        ...
        new("token_line_count", "1.0.0", ["solution", "project", "file", "type"], "syntax", "lines", "distinct source lines containing syntax tokens", "Counts lines with non-comment C# tokens in the target span or unique file population.", ["Preprocessor-only lines are not counted unless they contain syntax tokens."]),
        ...
        new("commented_line_count", "1.0.0", ["solution", "project", "file"], "syntax", "lines", "distinct lines intersecting ordinary or documentation comment trivia", "Counts all comment-bearing lines.", ["Overlaps token_line_count for mixed code/comment lines."]),
        ...
        new("token_line_ratio", "1.0.0", ["solution", "project", "file"], "derived", "ratio", "token_line_count / lines_of_code", "Reports the share of physical lines containing C# syntax tokens.", ["Descriptive only; not monotonic code quality."]),
        ...
        new("member_length", "1.0.0", ["member"], "syntax", "lines", "inclusive source line span count for the member declaration", "Counts source lines occupied by a member declaration.", ["Expression-bodied members can be one line even when semantically dense."]),
        ...
        new("dependency_cycle_membership", "1.0.0", ["type"], "semantic", "flag", "1 when the type belongs to a dependency SCC, otherwise 0", "Flags type dependency cycles from internal type dependency edges.", ["The MVP reports SCC membership, not an exact count of all simple cycles."]),
        new("distinct_outgoing_callee_count", "1.0.0", ["member", "type"], "semantic", "count", "distinct internal member callees", "Counts resolved internal callees from call graph edges.", ["Type targets aggregate distinct member callees across their members.", "Reflection, dynamic dispatch, framework callbacks and external calls are not represented."]),
        new("distinct_incoming_caller_count", "1.0.0", ["member", "type"], "semantic", "count", "distinct internal member callers", "Counts resolved internal callers from call graph edges.", ["Type targets aggregate distinct calling members across their members."]),
        ...
        new("type_coupling", "1.0.0", ["type"], "semantic", "count", "distinct non-special named types in bases, interfaces, attributes, signatures and type syntax", "Counts semantic type coupling across internal and external named types.", ["Primitive special types, type parameters, error types and the target type itself are excluded.", "Not a compatibility claim with Visual Studio or another CBO implementation."]),
